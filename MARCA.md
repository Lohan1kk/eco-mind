# EcoMind

**Consciência ambiental + tecnologia**

## Nome

**EcoMind** — “Eco” de meio ambiente e “Mind” de consciência. A marca une educação digital e cuidado com a natureza.

## Problema que resolvemos

**Desmatamento e perda de biodiversidade**, com ênfase nas **queimadas**.

Muita gente vê notícias de florestas destruídas e espécies ameaçadas, mas sente que não há o que fazer no dia a dia. Falta uma ponte entre a preocupação e ações concretas de conscientização — informação clara, hábitos de consumo e engajamento na escola e na comunidade.

A EcoMind cria essa ponte com um app que educa sobre causas e efeitos do desmatamento e das queimadas, propõe missões de conscientização e mostra caminhos práticos para cada pessoa agir.

## História

A EcoMind nasceu na escola. Em conversas e aulas, o grupo se deparou com a realidade dos **desmatamentos** e das **queimadas** — florestas destruídas, animais sem habitat e um futuro mais inseguro para a biodiversidade.

Em vez de só se indignar com as notícias, Ana Flávia (CEO e Design), Lucas Lohan (CTO / Programação), André Soares (Tecnologia / Programação) e Gabriel Rosa (Marketing) decidiram transformar essa preocupação em um projeto: um app que educa e conscientiza sobre o impacto do desmatamento e das queimadas, e mostra o que cada pessoa pode fazer.

A metáfora da marca é a **semente que vira broto**: pequenas ações e conhecimento plantados hoje podem crescer em mudança real. A EcoMind existe para cultivar essa consciência — na escola, em casa e na comunidade.

## Equipe

| Nome | Função |
|------|--------|
| **Ana Flávia** | CEO / Design
| **Lucas Lohan** | CTO / Programação (UX-UI)|
| **André Soares** | Tecnologia / Programação |
| **Gabriel Rosa** | Marketing / Conteúdo |

## Logo

- PNG: [`public/brand/icon-ecomind.png`](public/brand/icon-ecomind.png)
- Ícone maskable: [`public/brand/icon-ecomind-maskable.png`](public/brand/icon-ecomind-maskable.png)

Conceito: **natureza + mente** — metade folha, metade cérebro em circuito, unindo meio ambiente e consciência tecnológica. Gradiente verde-água a lima, em círculo.
